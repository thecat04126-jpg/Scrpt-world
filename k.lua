--!strict
-- DEBUG CONSOLE v8.0 — EXTENDED / INJECTOR-ORIENTED
-- Roblox / Executor
...
local Http = pcall(function() return game:GetService("HttpService") end) and game:GetService("HttpService") or nil
...
local function kv(k, v, col) log(("  %-32s : %s"):format(k, tostring(v)), col or C.val) end
...
-- ============================================================
-- ОПРЕДЕЛЕНИЕ ИНЖЕКТОРА / EXECUTOR
-- ============================================================
local EXECUTOR_KEYS = {
    -- identity / info
    ...
    "getconnections","getcustomasset","getsenv",
    -- closures / metatables
    ...
    "firetouchinterest","fireproximityprompt","fireclickdetector","firelclickdetector",
    "getconnections","firesignal","getcallbackvalue","getcallbackmember",
    ...
    -- signature flags
    "KRNL_LOADED","is_sirhurt_closure","fluxus","secure_load","isElectron",
    "isSynapse","is_sentinel","is_proto_smasher","get_script_hash",
    "sethiddenproperty","gethiddenproperty","sethiddenproperties",
    "isluau","getrawmetatable","getnamecallmethod","setnamecallmethod",
    "getcallbackvalue","setcallbackvalue",
}
...
local function detectExecutor()
    ...
    for _, k in ipairs(EXECUTOR_KEYS) do
        if rawget(_G, k) then present[#present+1] = k else missing[#missing+1] = k end
    end
    table.sort(present)
    ...
    -- Capabilities / идентичность
    sub("Идентичность / capabilities")
    for _, fn in ipairs({"getthreadidentity","getidentity","getthreadcontext"}) do
    ...
    for _, nk in ipairs({"request","http_request","syn","httpget","httppost"}) do
        kv(nk, rawget(_G, nk) and "есть" or "нет", rawget(_G, nk) and C.val or C.err)
    end
    if Http then
        pcall(function()
            local url = "https://httpbin.org/ip"
            local ok, resp = pcall(Http.GetAsync, Http, url)
            if ok then kv("HttpService (game)", "работает", C.val) else kv("HttpService (game)", "заблокирован", C.err) end
        end)
    end
end
...
local function detectAnticheat()
    ...
    pcall(function()
        ...
        if starter:FindFirstChild("StarterPlayerScripts") then
            for _, c in ipairs(starter.StarterPlayerScripts:GetChildren()) do
                if c.Name:lower():find("hyperion") or c.Name:lower():find("byfron") then
    ...
    for _, sn in ipairs({"AntiCheat","Byfron","Hyperion","RobloxScriptSecurity"}) do
    ...
    -- executor-специфичные античиты вроде SynapseBlock
    for _, suspect in ipairs({"syn","Sirhurt","Electron","Sentinel"}) do
        if rawget(_G, suspect) then flags[#flags+1] = "Executor flag: "..suspect end
    end
    ...
    kv("Hyperion (Byfron)", tostring(pcall(function() return game:GetService("Players").LocalPlayer end)), C.dim)
end
...
local function dumpGlobals()
    ...
    scanEnv("_G", _G, 250)
    if shared then scanEnv("shared", shared, 100) end
    if getgenv then
        local ok, g = pcall(getgenv)
        if ok and g and g ~= _G then scanEnv("getgenv()", g, 100) end
    end
    if getrenv then
        local ok, g = pcall(getrenv)
        if ok and g and g ~= _G then scanEnv("getrenv()", g, 60) end
    end
    if getfenv then
        local ok, e = pcall(getfenv, 0)
        if ok and e ~= _G then scanEnv("getfenv(0)", e, 80) end
    end
end
...
local function dumpServices()
    ...
    for _, c in ipairs(children) do
        if c:IsA("ServiceProvider") or pcall(function() return game:GetService(c.ClassName) end) then
    ...
    for _, s in ipairs(services) do
        local cnt = #s:GetChildren()
        log(("    %-38s  [%s]  детей=%d"):format(s.Name, s.ClassName, cnt), C.txt)
    end

    sub("Hidden / RobloxScriptSecurity сервисы (попытка)")
    for _, name in ipairs({"CoreGui","RobloxScriptSecurity","RobloxReplicatedStorage","ScriptContext","VirtualUser","AdService","StudioService","CorePackages","RobloxPluginGuiService","PluginGuiService","BrowserService","GuiService"}) do
...
local function serverInfo()
    ...
    local gk = {
        ...
        "PlaceVersion","Workspace","Name","ClassName",
    }
    ...
    if UIS then
        kv("Platform", tostring(UIS:GetPlatform and UIS:GetPlatform() or "?"))
    end
    pcall(function()
        kv("Roblox version", tostring(version and version() or "?"))
    end)
    ...
    if LP then
        for _, k in ipairs({"Name","DisplayName","UserId","AccountAge","MembershipType","Team","FollowUserId"}) do
            ...
            kv("  "..k, ok and tostring(v) or "?", C.val)
        end
        if LP.Character then
            ...
            if hum then
                ...
                kv("  State", tostring(hum:GetState()))
            end
        else
    ...
    for i, p in ipairs(PL:GetPlayers()) do
        log(("  [%02d] %-22s  Id=%-12d  Age=%-5d  Team=%s"):format(i, p.Name, p.UserId, p.AccountAge, p.Team and p.Team.Name or "—"), C.txt)
    end
end
...
local function performance()
    ...
    conn = RS.RenderStepped:Connect(function()
        frames += 1
        if frames >= 30 or os.clock() - t0 > 1 then
    ...
    pcall(function()
        kv("Workspace:GetRealPhysicsFPS()", tostring(workspace:GetRealPhysicsFPS()))
    end)
    pcall(function()
        kv("Lighting.ClockTime", tostring(game:GetService("Lighting").ClockTime))
    end)
end
...
-- ============================================================
local function scanInstances(label, root, maxDepth, filterClass)
    if not root then return end
    maxDepth = maxDepth or 3
    local function rec(inst, depth)
        if depth > maxDepth then return end
        if filterClass and inst:IsA(filterClass) then
            log(("  %s%s [%s]"):format(string.rep("  ", depth), inst.Name, inst.ClassName), C.txt)
        end
        for _, c in ipairs(inst:GetChildren()) do rec(c, depth + 1) end
    end
    sub(label)
    rec(root, 0)
end
...
local function dumpRemoteInfo()
    ...
    local function scanRemote(inst)
        if inst:IsA("RemoteEvent") then
            re += 1
            reList[#reList+1] = inst:GetFullName().." (RE)"
        elseif inst:IsA("RemoteFunction") then
            rf += 1
            reList[#reList+1] = inst:GetFullName().." (RF)"
        elseif inst:IsA("BindableEvent") then bp += 1 end
    end
    for _, d in ipairs(game:GetDescendants()) do scanRemote(d) end
...
local function dumpWorkspace()
    ...
    if count > 40 then log(("    ... ещё %d"):format(count - 40), C.dim) end

    sub("ServerScriptService / ServerStorage видимость")
    for _, sn in ipairs({"ServerScriptService","ServerStorage","ServerScriptService"}) do
        local ok, svc = pcall(function() return game:GetService(sn) end)
        if ok and svc then
...
local function findLib(n)
    ...
    if shared and shared[n] then return shared[n] end
    if getfenv then
        local ok, e = pcall(getfenv, 0)
...
local function collect()
    ...
    kv("Luau", _VERSION)
    kv("Скрипт", (script and script:GetFullName()) or "?")
...
local function buildGUI()
    ...
    crn(ic, 6)
    mk("TextLabel", {Size = UDim2.new(1, -260, 0, 40), Position = UDim2.fromOffset(44, 0), BackgroundTransparency = 1, Text = "DEBUG CONSOLE v8 — INJECTOR", TextColor3 = Color3.fromRGB(230, 235, 250), TextSize = 15, Font = Enum.Font.GothamBold, TextXAlignment = Enum.TextXAlignment.Left, Parent = tb})
    ...
    pcall(function()
        SG:SetCore("SendNotification", {Title = "Debug Console v8", Text = "F8 — показать/скрыть", Duration = 5})
    end)
end
